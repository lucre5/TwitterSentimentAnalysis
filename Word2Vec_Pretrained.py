...
model_2.build_vocab(sentences, progress_per=500)
total_examples = model_2.corpus_count
model = KeyedVectors.load_word2vec_format("/Users/lucrezialamanna/Desktop/glove.6B.300d.w2vformat.txt", binary=False)
model_2.build_vocab([list(model.vocab.keys())], update=True)
model_2.intersect_word2vec_format("/Users/lucrezialamanna/Desktop/glove.6B.300d.w2vformat.txt", binary=False, lockf=1.0)
model_2.train(sentences, total_examples=total_examples, epochs=model_2.iter)

# fit a 2d PCA model to the vectors
X = model_2[w2v_model.wv.vocab]
...
#fname = get_tmpfile("/Users/lucrezialamanna/Desktop/vectors_Retro_pred.txt")
...
word_vectors=model_2.wv
fname = get_tmpfile("vectorsP.kv")
word_vectors.save(fname)
word_vectors = KeyedVectors.load(fname)
word_vectors.save_word2vec_format('vec_fileP.txt')


# In[82]:


model_final = KeyedVectors.load_word2vec_format("/Users/lucrezialamanna/Desktop/out_vec.txt", binary=False)


# In[83]:


model_final.wv.most_similar(positive=["corbyn"])
...
mean_vec_tr = MeanEmbeddingVectorizer(model_final)
...
Train_X, Test_X, Train_Y, Test_Y = model_selection.train_test_split(doc_vec, df['Category'], random_state=5, test_size=0.3, stratify=df['Category'])
...
SVM = svm.SVC(C=1.0, kernel='linear', random_state=20)
...
clf_rf = RandomForestClassifier(n_estimators=25, random_state=20, class_weight= "balanced")
...
tfidf_vec_tr = TfidfEmbeddingVectorizer(model_final)
...
Train_X, Test_X, Train_Y, Test_Y = model_selection.train_test_split(tfidf_doc_vec, df['Category'], random_state=8, test_size=0.3, stratify=df['Category'])
...
print("SVM F1 Score -> ",f1_score(predictions_SVM_TF, Test_Y, average=None)*100)
